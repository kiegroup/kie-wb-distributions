...
import static org.kie.smoke.wb.base.util.RestUtil.get;
import static org.kie.smoke.wb.base.util.RestUtil.post;
import static org.kie.smoke.wb.base.util.TestConstants.ARTIFACT_ID;
import static org.kie.smoke.wb.base.util.TestConstants.GROUP_ASSSIGNMENT_PROCESS_ID;
...
import static org.kie.smoke.wb.base.util.TestConstants.OBJECT_VARIABLE_PROCESS_ID;
import static org.kie.smoke.wb.base.util.TestConstants.RULE_TASK_PROCESS_ID;
import static org.kie.smoke.wb.base.util.TestConstants.SALA_PASSWORD;
import static org.kie.smoke.wb.base.util.TestConstants.SALA_USER;
import static org.kie.smoke.wb.base.util.TestConstants.SCRIPT_TASK_VAR_PROCESS_ID;
...
public class KieRemoteRestTestMethods {

    public static final String deploymentId = KJAR_DEPLOYMENT_ID;
    public static final KModuleDeploymentUnit deploymentUnit = new KModuleDeploymentUnit(GROUP_ID, ARTIFACT_ID, VERSION);
    public static final String taskUserId = "salaboy";
    public static final Logger logger = LoggerFactory.getLogger(KieRemoteRestTestMethods.class);

    public static void doRestHistoryLogs( URL deploymentUrl, String mediaType ) throws Exception {
        ...
        String password = MARY_PASSWORD;
        {
            ...
            JaxbProcessInstanceResponse processInstance = post(deploymentUrl,
                    "rest/runtime/" + deploymentId + "/process/" + SCRIPT_TASK_VAR_PROCESS_ID + "/start?map_x=initVal", mediaType,
                    200, user, password, JaxbProcessInstanceResponse.class);
            long scriptTaskVarProcInstId = processInstance.getId();

            // instances/
            JaxbHistoryLogList historyLogList = get(deploymentUrl, "rest/history/instances", mediaType, 200, user, password,
            ...
            JaxbProcessInstanceLog procInstLog = get(deploymentUrl,
                    "rest/history/instance/" + origProcInstLog.getProcessInstanceId(), mediaType, 200, user, password,
                    JaxbProcessInstanceLog.class);
            assertNotNull("Null process instance log!", procInstLog);
            assertEquals("Log process instance id", procInstId, procInstLog.getProcessInstanceId().longValue());
            assertEquals("Process instance status", origProcInstLog.getStatus(), procInstLog.getStatus());

            // TODO: instance/{procInstId}/child

            // instance/{procInstId}/node
            historyLogList = get(deploymentUrl, "rest/history/instance/" + procInstId + "/node", mediaType, 200, user, password,
                    JaxbHistoryLogList.class);
            ...
            historyLogList = get(deploymentUrl, "rest/history/instance/" + scriptTaskVarProcInstId + "/variable/x", mediaType, 200,
                    user, password, JaxbHistoryLogList.class);
        ...
        // process/{procDefId}
        {
            JaxbProcessDefinition procDef = get(deploymentUrl,
                    "rest/runtime/" + deploymentId + "/process/" + OBJECT_VARIABLE_PROCESS_ID, mediaType, 200, user, password,
...

        {
            ...
            String varVal = "10";
            JaxbProcessInstanceResponse procInstResp = post(deploymentUrl, "rest/runtime/" + deploymentId + "/process/"
                    + OBJECT_VARIABLE_PROCESS_ID + "/start?map_" + varId + "=" + varVal, mediaType, 200, user, password,
                    JaxbProcessInstanceResponse.class);
            long objVarProcInstId = procInstResp.getResult().getId();

            // variable/{varId}
            JaxbHistoryLogList jhll = get(deploymentUrl, "rest/history/variable/" + varId, mediaType, 200, user, password,
                    JaxbHistoryLogList.class);
            ...
            jhll = get(deploymentUrl, "rest/history/variable/" + varId + "/instances", mediaType, 200, user, password,
                    JaxbHistoryLogList.class);
    ...
    public static void doRestRemoteApiExtraJaxbClasses( URL deploymentUrl, String mediaType ) throws Exception {
        String user = MARY_USER;
        String password = MARY_PASSWORD;
    ...
    public static void doRestRemoteApiHumanTaskProcess( URL deploymentUrl, String mediaType ) throws Exception {
    ...
    public static void doRestRemoteApiRuleTaskProcess( URL deploymentUrl, String mediaType ) throws Exception {
        // Remote API setup
        ...
        List<VariableInstanceLog> varLogs = (List<VariableInstanceLog>) auditService.findVariableInstancesByName("requestReason",
                false);
    ...
    public static void doRestUrlsGroupAssignmentProcess( URL deploymentUrl, String mediaType ) throws Exception {
        JaxbProcessInstanceResponse procInstResp = post(deploymentUrl,
                "rest/runtime/" + deploymentId + "/process/" + GROUP_ASSSIGNMENT_PROCESS_ID + "/start", mediaType, 200, MARY_USER,
                MARY_PASSWORD, JaxbProcessInstanceResponse.class);
        assertEquals(ProcessInstance.STATE_ACTIVE, procInstResp.getState());
        long procInstId = procInstResp.getId();

        // assert the task
        TaskSummary taskSummary = getTaskSummary(deploymentUrl, procInstId, Status.Ready, MARY_USER, MARY_PASSWORD, mediaType);
        ...
        post(deploymentUrl, "rest/task/" + taskId + "/claim", mediaType, 200, MARY_USER, MARY_PASSWORD);
        post(deploymentUrl, "rest/task/" + taskId + "/start", mediaType, 200, MARY_USER, MARY_PASSWORD);
        post(deploymentUrl, "rest/task/" + taskId + "/complete", mediaType, 200, MARY_USER, MARY_PASSWORD);

        // now make sure that the next task has been assigned to the
        // correct person. it should be mary.
        taskSummary = getTaskSummary(deploymentUrl, procInstId, Status.Reserved, MARY_USER, MARY_PASSWORD, mediaType);
        ...
        post(deploymentUrl, "rest/task/" + taskId + "/release", mediaType, 200, MARY_USER, MARY_PASSWORD);
        post(deploymentUrl, "rest/task/" + taskId + "/start", mediaType, 200, JOHN_USER, JOHN_PASSWORD);
        post(deploymentUrl, "rest/task/" + taskId + "/complete", mediaType, 200, JOHN_USER, JOHN_PASSWORD);

        // now make sure that the next task has been assigned to the
        // correct person. it should be john.
        taskSummary = getTaskSummary(deploymentUrl, procInstId, Status.Reserved, JOHN_USER, JOHN_PASSWORD, mediaType);
        ...
        JaxbProcessInstanceLog jaxbProcInstLog = get(deploymentUrl, "rest/history/instance/" + procInstId, mediaType, 200,
                MARY_USER, MARY_PASSWORD, JaxbProcessInstanceLog.class);
    ...
    public static void doUrlsGetDeployments( URL deploymentUrl, String mediaType ) throws Exception {
        ...
        JaxbDeploymentUnit dep = get(deploymentUrl, "rest/deployment/" + deploymentId, mediaType, 200, user, password,
    ...
    protected static long findTaskId( Long procInstId, List<TaskSummary> taskSumList ) {
        long taskId = -1;
    ...
    private static String getConnectionContent( Object content ) throws Exception {
    ...
    private static RuntimeEngine getRemoteRuntime( URL deploymentUrl, String user, String password ) {
        // @formatter:off
    ...
    private static TaskSummary getTaskSummary( URL deploymentUrl, long processInstanceId, Status status, String user,
            String password, String mediaType ) throws Exception {
        JaxbTaskSummaryListResponse taskSumListResp = get(deploymentUrl,
                "rest/task/query?processInstanceId=" + processInstanceId + "&status=" + status.toString(), mediaType, 200, user,
                password, JaxbTaskSummaryListResponse.class);
        List<TaskSummary> taskSumList = taskSumListResp.getResult();
        assertEquals(1, taskSumList.size());
        return taskSumList.get(0);
    }

    private static void testParamSerialization( RuntimeEngine engine, Object param ) {
